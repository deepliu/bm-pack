...
- 允许混装（一个箱可装多个 SKU）。
- 允许旋转（默认允许 6 向旋转，可配置关闭）。
- 核心目标：**箱子数量最少**。
- 约束：每箱总重必须满足 **12.0 kg ≤ weight ≤ 22.5 kg**（可配置）。
...
## 3. 求解策略（分层求解，先最少箱数，再修复重量，再校验尺寸）

### Stage A：重量驱动的最少箱数分箱（主解）
- 先从 B_min 个箱开始尝试装（不足再加箱）。
- 物品按 weight 降序（或 weight*volume 降序）展开排序。
- 采用 Best-Fit（或 Best-Fit Decreasing）：
  - 选择放入后最接近 max_weight 且不超过 max_weight 的箱。
  - 同时做快速尺寸过滤（仅做单件能放下 + 体积占比阈值 fill_rate 默认 0.90）。

### Stage B：修复 underweight（< min_weight）
- 处理所有 weight < min_weight 的箱：
  1) 两个轻箱尝试合并（<= max_weight 且尺寸快检通过）— 优先，能减少箱数
